...
class webGalleryThumbnail {
    ...
    *@return boolean
    **/
	function webGalleryThumbnail($imgfile) 	{
    ...
    **/
	function save($save="")	{
        ...
        ob_start();
        if ($this->output_format=="PNG") { //PNG
    	    if (!imagePNG($this->img["des"], NULL)) {
    	    	ob_end_clean();
    	    	return false;
    	    }
    	} else {
           imageinterlace( $this->img["des"], $this->jpeg_progressive);
           if (!imageJPEG($this->img["des"], NULL, $this->quality)) {
           	ob_end_clean();
           	return false;
           }
        }
        ...
        if ($img_to_save) return $img_to_save;
    ...
    **/
    function unique_filename ( $archive_dir , $filename , $file_prefix='') {
        ...
        /**
        * NOT USED : to do: mezclar imagenes a tamaĹ„o original, preservar canal alpha y redimensionar
