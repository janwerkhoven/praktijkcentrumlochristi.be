	...
	*/
	$.fn.boxplusTransition = function (settings) {
		...
		lists.each(function () {
			...
			// DOM elements
			var list = $(this).wrap('<div />').before('<div />').css('visibility', 'hidden');
			var wrapper = list.parent().addClass('boxplus-transition-wrapper');
			var items = $('li', list).find('img:first');
			var container = list.prev().addClass('boxplus-transition').addClass(CLASS_DISABLED);
			...
			_updatePaging();
			container.click(open).removeClass(CLASS_DISABLED);
			scrollFirst();
			...
			function open() {
				items.eq(sliderIndexPosition).click();
			}
