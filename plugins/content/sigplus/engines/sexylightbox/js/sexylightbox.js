...
(function($) {
  $.bind = function(object, method){
    ...
    return function() {
      var args2 = [this].concat(args, jQuery.makeArray( arguments ));  // fix by Levente Hunyadi
  ...
  SexyLightbox = {
    ...
    /*
    Function: replaceBox
    @description  Cambiar de tamańo y posicionar el lightbox en el centro de la pantalla
    */
    ...
    getInfo: function (image, id) {
      image=$(image);
      IEuta = $('<a id="'+this.options.name+'-'+id+'" rel="'+image.attr('rel')+'">&nbsp;</a>');
      IEuta.css({ 'background-image' : 'url('+this.options.dir+'/'+this.options.color+'/'+this.options.buttons+')' });
      IEuta.attr('title', image.attr('title'));  // fix by Levente Hunyadi
