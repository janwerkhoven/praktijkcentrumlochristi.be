...
/**
* Extracts image relative URL from PATH_INFO.
*/
...
$imageurl = http_path_info_url();
if (empty($imageurl))
	http_critical_error('The image to download has not been specified in the URL.');

// check image existence
$filename = basename($imageurl);
$imagepath = JPATH_BASE.DS.str_replace('/', DS, $imageurl);
if (!is_file($imagepath))  // image file not found
	http_not_found($filename);
if (substr($imagepath, 0, strlen(JPATH_BASE.DS)) !== JPATH_BASE.DS)  // image path is outside Joomla folder
	http_not_found($filename);
